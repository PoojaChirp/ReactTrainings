...
/*   updateSyncDocument(formData) {
  ...
  updateSyncListItem(formData) {
    if (!this.client) {
      return;
    }
    //what happens when there's no list items yet?
...
/*   async loadFormData() {
  ...
  async loadFormData() {
    ...
    this.client.list(this.props.sessionId).then(function(list) {
      component.setState({formData: list.get(0)});

      //?????
      list.on('itemUpdated',function(item) {
        console.log('Sync Updated Item', item.value);
        if (!item.isLocal) {
          console.log('Setting state with', item.value);
          component.setState({formData: item.value});
        }
      });
...
/*   setFormValue(fieldName,value) {
  ...
  setFormValue(fieldName,value) {
    var formData = this.state.formData;
    formData[fieldName] = value;
    this.setState({formData: formData}, () => this.updateSyncListItem(formData));
  }
                ...
                <div className="card-header text-info">
                    <div className="input-group mb-3">
                      <SyncedInputField
                        ...
                        formDataKey="id" 
                        formDataValue={this.state.formData['id']} 
                        placeholder="ID"/>
                    </div>
                    <div className="input-group mb-3">
                      <SyncedInputField
                    ...
                    <div className="input-group mb-3">
                      <SyncedInputField
                        setFormValue={this.setFormValue}
                        formDataKey="aptNotes" 
                        formDataValue={this.state.formData['aptNotes']} 
